...
public class Database implements AutoCloseable {
    ...
    private class TransactionContextImpl extends TransactionContext {
        ...
        public void close() {
            try {
                // TODO(proj4_part2)
                List<Lock> listOfLock = lockManager.getLocks(this);
                for (int i = listOfLock.size() - 1; i >= 0; i--) {
                    LockContext toRelease = LockContext.fromResourceName(lockManager, listOfLock.get(i).name);
