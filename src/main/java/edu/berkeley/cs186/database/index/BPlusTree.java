...
public class BPlusTree {
    ...
    public Iterator<RecordId> scanAll() {
        ...
        LeafNode left_most = this.root.getLeftmostLeaf();
        return new BPlusTreeIterator(left_most, 0);
    }
    ...
    public Iterator<RecordId> scanGreaterEqual(DataBox key) {
        ...
        int index = InnerNode.numLessThan(key, leaf_node.getKeys());

        return new BPlusTreeIterator(leaf_node, index);
    }
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        Optional<Pair<DataBox, Long>> new_node = this.root.put(key, rid);
        if (!new_node.equals(Optional.empty())) {
            Pair<DataBox, Long> new_pair = new_node.get();
            DataBox new_root_key = new_pair.getFirst();
            Long right_node = new_pair.getSecond();

            List<DataBox> new_keys = new ArrayList<>();
            new_keys.add(new_root_key);

            List<Long> new_children = new ArrayList<>();
            new_children.add(this.root.getPage().getPageNum());
            new_children.add(right_node);
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        if (!root.getLeftmostLeaf().getRids().isEmpty()) throw new BPlusTreeException("The tree is NOT empty!");
        // start from the leafnode
        Optional<Pair<DataBox, Long>> result = root.bulkLoad(data, fillFactor);

        // check whether we need to split or not
        while (result.isPresent()) {
            // initialize the new root
            ArrayList<DataBox> key = new ArrayList<>();
            ArrayList<Long> children = new ArrayList<>();
            key.add(result.get().getFirst());
            children.add(root.getPage().getPageNum()); // left
            children.add(result.get().getSecond()); // right
            InnerNode newRoot = new InnerNode(metadata, bufferManager, key, children, lockContext);
            // update the root
            updateRoot(newRoot);
            // call bulkload repeatedly
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        LeafNode current_leaf = null;
        int index = 0;
        int leaf_size = 0;
        private BPlusTreeIterator(LeafNode leaf_node, int index) {
            this.current_leaf = leaf_node;
            this.index = index;
            this.leaf_size = this.current_leaf.getKeys().size();
        }

        @Override
        public boolean hasNext() {
            // TODO(proj2): implement
            if (this.index < this.leaf_size) {
                return true;
            } else if (this.index == this.leaf_size && this.current_leaf.getRightSibling().isPresent()) {
                return true;
            }
            return false;
        }
        ...
        public RecordId next() {
            ...
            RecordId next_id;
            if (this.index == this.leaf_size) {
                if (this.current_leaf.getRightSibling().equals(Optional.empty())) {
                    throw new NoSuchElementException();
                }
                LeafNode next_leaf = this.current_leaf.getRightSibling().get();
                this.current_leaf = next_leaf;
                this.index = 0;
                this.leaf_size = this.current_leaf.getKeys().size();
            }
            next_id = this.current_leaf.getRids().get(this.index);
            this.index += 1;
            return next_id;
        }
    }
