...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        // Case 2: If node overflow, then n is split into a left and right node
        //         and a pair (split_key, right_node_page_num)
        // If a duplicate key is inserted into a leaf node, the tree is left unchanged
        // and a BPlusTreeException is raised.

        // Find the index for insertion
        int i = 0;
        while (i < keys.size()) {
            DataBox current = keys.get(i);
            if (key.equals(current)) {
                throw new BPlusTreeException("Duplicate key is inserted into a leaf node!");
            } else if (key.compareTo(current) < 0) { // less than equal
                break;
            }
            i++;
        }
        ...
        keys.add(i, key);
        rids.add(i, rid);
        // decide whether we need to split the node
        if (keys.size() <= 2 * metadata.getOrder()) {
            sync();
            return Optional.empty();
        } else {
            // split into a left and right node
            List<DataBox> rightKeys = new ArrayList<>();
            List<RecordId> rightRids = new ArrayList<>();
            int order = metadata.getOrder();
            // move d+1 pairs from left to right node
            for (i = order; i < keys.size(); i++) {
                rightKeys.add(keys.get(i));
                rightRids.add(rids.get(i));
            }
            // remove d+1 pairs from left node
            keys.removeAll(rightKeys);
            rids.removeAll(rightRids);
            // initialize a new node
            LeafNode rightNode = new LeafNode(metadata, bufferManager, rightKeys, rightRids, rightSibling, treeContext);
            rightSibling = Optional.of(rightNode.page.getPageNum()); // who is right sibling? // should I split in here

            // update the change into disk
            rightNode.sync();
            this.sync();
            return Optional.of(new Pair<>(rightKeys.get(0), rightNode.page.getPageNum())); // how to get page num
        }
        // return Optional.empty();
    }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
         ...
         */
        int maxOccupancy = (int) Math.ceil(2 * metadata.getOrder() * fillFactor);

        // fill up until maxOccupancy or no more node
        while (data.hasNext() && keys.size() < maxOccupancy) {
            Pair<DataBox, RecordId> nextData = data.next();
            keys.add(nextData.getFirst());
            rids.add(nextData.getSecond());
        }
        // create a right sibling that contains just one record
        if (data.hasNext()) {
            ...
            LeafNode rightNode = new LeafNode(metadata, bufferManager, rightKeys, rightRids, Optional.empty(), treeContext);
            rightNode.sync();
            // update the current node
            rightSibling = Optional.of(rightNode.page.getPageNum());
            sync();
            // copy up the value to the parent
            return Optional.of(new Pair<>(rightKeys.get(0), rightNode.page.getPageNum()));
        }
        sync();
        return Optional.empty();
    }
    ...
    public void remove(DataBox key) {
        ...
        // sanity check
        if (key == null || keys.size() == 0) return;
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        // brand new one.
        // obtain the page from disk based on page number
        if (metadata == null || bufferManager == null || treeContext == null) {
            return null;
        }
        ...
        // store the key and rids pair from disk
