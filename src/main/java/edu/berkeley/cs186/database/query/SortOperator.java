...
public class SortOperator extends QueryOperator {
    ...
    public Run sortRun(Iterator<Record> records) {
        ...
        List<Record> temp_records = new ArrayList<>();
        while (records.hasNext()) {
            temp_records.add(records.next());
        }
        Collections.sort(temp_records, this.comparator);

        Run new_run = new Run(this.transaction, this.computeSchema());
        new_run.addAll(temp_records);
        return new_run;
    }
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        for (int i = 0; i < runs.size(); i++) {
            ...
            if (iters.get(i).hasNext()) {
                Record next = iters.get(i).next();
                pq.add(new Pair(next, i));
            }
        }

        while(true) {
            Pair<Record, Integer> temp = pq.poll();
            Record temp_rec = temp.getFirst();
            int index = temp.getSecond();

            new_run.add(temp_rec);
            if (iters.get(index).hasNext()) {
                pq.add(new Pair(iters.get(index).next(), index));
            }
            if (pq.isEmpty()) {
                break;
            }
    ...
    public List<Run> mergePass(List<Run> runs) {
        // TODO(proj3_part1): implement
        List<Run> new_pass = new ArrayList<>();

        int count = 0;
        while (count + numBuffers - 1 < runs.size()) {
            List<Run> temp = new ArrayList<>();
            for (int i = count; i < count + numBuffers - 1; i++) {
                temp.add(runs.get(i));
            }
            Run new_run = mergeSortedRuns(temp);
            new_pass.add(new_run);
            count += (numBuffers - 1);
        }
        if (count < runs.size() - 1) {
            List<Run> temp = new ArrayList<>();
            for (int i = count; i < runs.size(); i++) {
                temp.add(runs.get(i));
            }
            Run new_run = mergeSortedRuns(temp);
            new_pass.add(new_run);
    ...
    public Run sort() {
        ...
        while(sourceIterator.hasNext()) {
            Run new_run = sortRun(sourceIterator);
            sorted_runs.add(new_run);
        }
        List<Run> passes = sorted_runs;
        while (passes.size() > 1) {
