...
public class GHJOperator extends JoinOperator {
    ...
    public static Pair<List<Record>, List<Record>> getBreakSHJInputs() {
        ...
        int max_capable_size = (6-2)*10*5;
        int overflow_size = max_capable_size + 50;
        for (int i = 0; i < overflow_size; i++) {
            Record new_record = createRecord(i);
            leftRecords.add(new_record);
        }
        int ok_size = max_capable_size - 50;
        for (int j = 0; j < ok_size; j++) {
            Record new_record = createRecord(j);
            rightRecords.add(new_record);
        }
    ...
    public static Pair<List<Record>, List<Record>> getBreakGHJInputs() {
        ...
        int overflow_size = max_capable_size + 50;
        for (int i = 0; i < overflow_size; i++) {
            Record new_record = createRecord(0);
            leftRecords.add(new_record);
        }
        int ok_size = max_capable_size - 50;
        for (int j = 0; j < ok_size; j++) {
