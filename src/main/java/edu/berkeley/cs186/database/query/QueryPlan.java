...
public class QueryPlan {
    ...
    /**
     * For each table in this.joinTableNames
     * - creates a minCost operator over the table
     * - joins the current final operator with the operator
     * - sets the final operator to the join
     */
    ...
    public QueryOperator minCostSingleAccess(String table) {
        if (!this.aliases.containsKey(table)) {
            return null;
        }
        QueryOperator minOp = new SequentialScanOperator(this.transaction, table);

        // TODO(proj3_part2): implement
        int seqScanEstimatedCost = minOp.estimateIOCost();
        int minCost = seqScanEstimatedCost;
        int minIndex = -1;

        List<Integer> eligibleIndexColumns = getEligibleIndexColumns(table);
        for (int i = 0; i < eligibleIndexColumns.size(); i++) {
            SelectPredicate curr = this.selectPredicates.get(i);
            IndexScanOperator curOp = new IndexScanOperator(this.transaction, table, curr.column, curr.operator, curr.value);
            int curEstimatedCost = curOp.estimateIOCost();
            if (minCost > curEstimatedCost) {
                minCost = curEstimatedCost;
                minOp = curOp;
                minIndex = i;
            }
        }
    ...
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        if (prevMap == null || prevMap.size() == 0 || pass1Map == null || pass1Map.size() == 0) {
            return result;
        }
        for (Set childTableSet : prevMap.keySet()) {
            for (int i = 0; i < joinPredicates.size(); i++) {
                String leftTable = joinPredicates.get(i).leftTable;
                String leftColumn = joinPredicates.get(i).leftColumn;
                String rightTable = joinPredicates.get(i).rightTable;
                String rightColumn = joinPredicates.get(i).rightColumn;
                boolean containLeft = childTableSet.contains(leftTable);
                boolean containRight = childTableSet.contains(rightTable);
                Set prevJoinSet = new HashSet();
                QueryOperator prevOperator = prevMap.get(childTableSet);
                QueryOperator newOperator = null;
                if (containLeft && !containRight) {
                    prevJoinSet.add(rightTable);
                    newOperator = pass1Map.get(prevJoinSet);
                } else if (!containLeft && containRight) {
                    prevJoinSet.add(leftTable);
                    newOperator = pass1Map.get(prevJoinSet);
                    leftColumn = joinPredicates.get(i).rightColumn;
                    rightColumn = joinPredicates.get(i).leftColumn;
                } else {
                    continue;
                }
                prevJoinSet.addAll(childTableSet);
                QueryOperator newTableOperator = minCostJoinType(prevOperator, newOperator, leftColumn, rightColumn);
                System.out.println("newTableOperator IO COST: "  + newTableOperator.estimateIOCost() + " \nOperator" + newTableOperator);
                // update the result map if needed
                if (result.containsKey(prevJoinSet)) {
                    if (newTableOperator.estimateIOCost() < result.get(prevJoinSet).estimateIOCost())
                        result.put(prevJoinSet, newTableOperator);
                } else {
                    result.put(prevJoinSet, newTableOperator);
                }
            }
    ...
    public Iterator<Record> execute() {
        ...
        else if (this.aliases.size() == 1) {
            for (String alias: this.aliases.keySet()) {
                this.finalOperator = minCostSingleAccess(alias);
                this.addGroupBy();
                this.addProject();
                this.addSort();
                this.addLimit();
        ...
        for (String alias: this.aliases.keySet()) {
            ...
            table.add(alias);
            pass1map.put(table, minCostSingleAccess(alias));
        }

        Map<Set<String>, QueryOperator> prevMap = minCostJoins(pass1map, pass1map);

        while (prevMap.size() > 1) {
            prevMap = minCostJoins(prevMap, pass1map);
        }
        for (Set key: prevMap.keySet()) {
            this.finalOperator = prevMap.get(key);
        }
        this.addGroupBy();
        this.addProject();
        this.addSort();
