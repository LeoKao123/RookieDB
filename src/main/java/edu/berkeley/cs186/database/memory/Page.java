...
public class Page {
    ...
    private class PageBuffer extends AbstractBuffer {
        ...
        public Buffer get(byte[] dst, int offset, int length) {
            ...
            LockUtil.ensureSufficientLockHeld(lockContext, LockType.S); // change this from NL to S for task 5
        ...
        public Buffer put(byte[] src, int offset, int length) {
            // TODO(proj4_part2): Update the following line
            LockUtil.ensureSufficientLockHeld(lockContext, LockType.X); // change this from NL to X for task 5
